...
class DifficultyDialog(simpledialog.Dialog):
    def body(self, master):
        ...
        opts = [("Facile (1m30)", "Facile"), 
                ("Moyen (1m)", "Moyen"), 
                ("Difficile (30s)", "Difficile")]
...
class MotusGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Motus Python Elite")
        self.root.configure(bg="#f0f2f5")
        
        # 1. Demander la longueur
        self.longueur = self.demander_longueur()
        if not self.longueur:
            self.root.destroy()
            return
            
        # 2. Demander la difficulté
        self.difficulte = self.demander_difficulte()
        if not self.difficulte:
            self.root.destroy()
            return
            
        # Réglage du temps selon difficulté
        ...
        if not chemin_dico:
            messagebox.showerror("Erreur", f"Dictionnaire pour {self.longueur} lettres introuvable.")
            self.root.destroy()
            return

        try:
            self.jeu = MotusEngine(chemin_dico, tentatives_max=7)
        ...
        self.root.geometry(f"{max(450, self.longueur * 50)}x650")
        self.creer_interface()
        self.afficher_indices()
        self.update_timer()

    def demander_longueur(self):
        return simpledialog.askinteger("Configuration", "Longueur du mot (2-15) :", 
    ...
    def creer_interface(self):
        ...
        tk.Label(header_frame, text=f"MOTUS - {self.difficulte.upper()}", fg="white", bg="#1a73e8",
                 font=("Segoe UI", 24, "bold")).pack(pady=10)
        
        # Timer Label
        self.lbl_timer = tk.Label(self.root, text=f"Temps : {self.temps_restant}s", 
                                 font=("Segoe UI", 16, "bold"), bg="#f0f2f5", fg="#d93025")
        ...
        for r in range(self.jeu.tentatives_max):
            ...
            for c in range(self.jeu.taille):
                lbl = tk.Label(self.grid_frame, text=".", width=3, height=1, 
                               relief="flat", font=("Consolas", 20, "bold"),
                               bg="white", fg="#bdc1c6")
        ...
        self.entree = tk.Entry(control_frame, font=("Segoe UI", 18), 
                               width=self.longueur + 2, justify="center",
        ...
        self.btn = tk.Button(control_frame, text="VALIDER", command=self.valider,
    ...
    def update_timer(self):
        if self.timer_running and self.temps_restant > 0:
            ...
            messagebox.showinfo("Temps écoulé", f"Dommage ! Le temps est fini.\nLe mot était : {self.jeu.secret}")
            self.root.destroy()
    ...
    def afficher_indices(self):
        ...
        for i, lettre in enumerate(etat["indices_decouverts"]):
            if row < self.jeu.tentatives_max:
    ...
    def valider(self):
        ...
        if res == "WRONG_SIZE":
            messagebox.showwarning("Attention", f"Il faut {self.jeu.taille} lettres !")
            return
        elif res == "NOT_IN_DICT":
            messagebox.showwarning("Inconnu", f"'{mot}' n'est pas dans le dictionnaire.")
            return
        ...
        for i, lettre in enumerate(res):
            ...
            if lettre.isupper():
                lbl.config(text=lettre, bg="#d93025", fg="white")
            elif lettre.islower():
                lbl.config(text=lettre.upper(), bg="#f9ab00", fg="white")
            else:
                lbl.config(text=mot[i], bg="#5f6368", fg="white")
                
        etat = self.jeu.obtenir_etat()
        if etat["termine"]:
            self.timer_running = False
            if etat["gagne"]:
                messagebox.showinfo("Gagné !", f"Félicitations !\nMot trouvé : {etat['secret']}\nTemps restant : {self.temps_restant}s")
            else:
                messagebox.showinfo("Perdu", f"Essais épuisés !\nLe mot était : {etat['secret']}")
            self.root.destroy()
        else:
            self.afficher_indices()
